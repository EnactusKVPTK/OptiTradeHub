...
const generateJwt = (id, email, name, role) => {
    return jwt.sign({id, email, name, role}, 
...
class UserController {
    async registration(req, res, next){
        ...
        if (!name || !email || !password){
            return next(ApiError.badRequest('Incorrect email, name or password'))
        }
        const candidate = await User.findOne({where: {email}})
        const candidateName = await User.findOne({where: {name}})
        if(candidate){
            return next(ApiError.badRequest('User with this email or name already exists'))
        }
        if(candidateName){
            return next(ApiError.badRequest('User with this name or name already exists'))
        }
    ...
    async login(req, res, next){
        ...
        if (!comparePassword){
            return next(ApiError.internal('Wrong passord'))
        }
        const token = generateJwt(user.id, user.email, user.name, user.role)
        return res.json(token)
    }
